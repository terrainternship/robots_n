...
def turn_numbers_in_text_into_int(russian_number):
    words = russian_number.split()
    results = []
    number = 0
    current_number = 0
...
"""
def calculate_similarity(test_numbers, number_words_base):
    for test_number in test_numbers:
        print(f"Схожесть для '{test_number}':")
        for word, number in number_words_base.items():
            similarity = fuzz.ratio(test_number, word)
            print(f"{word} ({number}): {similarity}%")
...
calculate_similarity(test_numbers, number_words_base)
"""
